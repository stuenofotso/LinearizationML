...
object MultiLinearizationClassificationExp {
  ...
  def main(args: Array[String]): Unit = {
    processLinearizationClassificationExp("/Users/steve/Downloads/cod-rna.txt", 0.65)
  ...
  *  YProba0:
   */
  def probabilityQuest(x:Row, Yprime: Dataset[YPrimeExp],  allowedToViolate: Dataset[String]): YPrimeExp = {

    //val Yprime = Yprime1.toDF("count", "yre", "yprime", "yProba", "xStr", "changed")

    //println("Yprime = "+Yprime)

    val yprime = Yprime.filter($"xStr".equalTo(lit(x.mkString(",")))).first()
  ...
  def learn(X: DataFrame, Yprime: Dataset[YPrimeExp], redo: Boolean, iter: Int): (Dataset[YPrimeExp], Boolean) = {
  ...
  def processLinearizationClassificationExp(filePath:String, percentage: Double): Unit = {
    ...
    val Yprime = X.map(row=>YPrimeExp(1, row.getDouble(0), Util.prime(row), if (row.getDouble(0) == 0.0) (scala.util.Random.nextDouble() * 2) / 5 else ((scala.util.Random.nextDouble() * 2) + 3) / 5, row.mkString(","), changed = false)).cache()
    ...
    X.take(20).map(row => (row.mkString(","), predict(row, model._1))).foreach(t => println("x = " + t._1  +  " et ypre = " + t._2))



    val wellClassifiedCount = XTest.collect().par.count(r => predict(r, model._1) == r.getDouble(0))
    ...
    val wellClassifiedCountX = X.collect().par.count(r => predict(r, model._1) == r.getDouble(0))
    val countX = X.count()
  ...
  def predict(x: Row,  Yprime: Dataset[YPrimeExp]): Double = {
  ...
  def learn(x: Row,  Yprime: Dataset[YPrimeExp]): Double = {
