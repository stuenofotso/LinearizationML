...
/** Main class */
object MultiLinearizationClassificationExp {
  ...
  def main(args: Array[String]): Unit = {
    processLinearizationClassificationExp(1000, Util.M, 0.6)
  }
  ...
  *  YProba0:
   */
  def probabilityQuest(i: Int, X: Array[Array[Double]], Y: Array[Double], Yprime: Array[Double], YProba: Array[Double], allowedToViolate: List[Int], yProba0: Double, redo: Boolean): (Array[Double], Boolean) = {
  ...
  def learn(X: Array[Array[Double]], Y: Array[Double], Yprime: Array[Double], YProba: Array[Double], redo: Boolean, iter: Int): (Array[Double], Boolean) = {

    val oldProba = YProba.clone()


    val res = probabilityQuest(0, X, Y, Yprime, YProba, scala.util.Random.shuffle(0 to X.size - 1).toList.take(Math.round(X.size * NumberConsensusPrior).toInt), learn(0, X, Yprime, YProba), redo = false)
  ...
  def processLinearizationClassificationExp(N: Int, M:Int, percentage: Double) = {
    ...
    scala.util.Random.shuffle(0 to X.size - 1).toList.take(20).map(i => (i, predict(X(i), Yprime, model._1))).foreach(t => println("x = " + X(t._1).mkString(",") + ", yre = " + Y(t._1) + " et ypre = " + t._2))
  ...
  def predict(x: Array[Double], Yprime: Array[Double], YProba: Array[Double]): Double = {
  ...
  def learn(i: Int, X: Array[Array[Double]], Yprime: Array[Double], YProba: Array[Double]): Double = {
